...
for run_number in range(50):
    ...
    xy_cells = 1024
    n = RandomTissue(xy_cells, -1, wavelength, dx, dz, n_h, ls, g, 300)
    np.save('refractive_index_HG', n)
    res_index = 0
    for Total_length in [5e-6, 10e-6, 25e-6, 45e-6, 65e-6, 85e-6, 120e-6, 150e-6]:
    
        # Simulation parameters
        
        
        NA = 2/15
        beam_radius = NA*Total_length
        focal_length = Total_length/1.33 # Focal length of lens used to focus
        #Total_length = 150e-6
        
        
        beam_type = 'HG' # 'HG, 'LG', 'G'
        Used_picked_index = True
        ...
        #unique_layers=int(Total_length/(5*dz))   # Unique layers of biological tissue. Repeats the same random slice of tissue after this many steps.
        ...
        if Used_picked_index:
            n = np.load('refractive_index_HG.npy')
        ...
        if beam_type=='LG':
            ...
            f.suptitle([beam_type, l])
        else:
            for i in range(np.shape(Field_snapshots)[2]):
                Field_snapshots[:,:,i] = np.abs(Field_snapshots[:,:,i])**2+np.abs(Field_snapshots[:,:,i].transpose())**2
            f = PlotSnapshots(Field_snapshots, imaging_depth)    
            f.suptitle([beam_type, u, v])
            
        print('Focal length is %1.1f um' %(focal_length*10**6))
        results[0,res_index,run_index] = Total_length*10**6
        results[1,res_index,run_index],results[2,res_index,run_index] = VortexNull(Field_snapshots[:,:,3], dx, cross_sections = 20, num_samples = 100)
        res_index = res_index + 1
    run_index = run_index+1
    print("--- %s seconds ---" % '%.2f'%(time.time() - start_time))
np.save('results_HG', results)