...
num_runs = 1           # Simulate each depth this many times
...
depths = np.array([5e-6,10e-6,20e-6,30e-6,40e-6,50e-6,60e-6,70e-6,80e-6])
...
def LG_donut_PSF(args):
    depth = args[0]
    run_number = args[1]
    LG_beam_order_number = args[2]  # Mode order number. 0 is gaussian, 1 is donut, 2 is higher order donut
    ...
    seed_x = LG_OAM_beam(xy_cells, seed_dx, beam_radius, LG_beam_order_number)
    seed_y = 1j*seed_x
    ...
    plot_LG(focus_depth,beam_radius,n_h,wavelength,xy_cells,FDFD_dx,LG_Focus_Intensity,depth,run_number,LG_beam_order_number)
...
if __name__ == '__main__':
    ...
    for run_number in range(num_runs):
        for depth in depths:
            args.append([depth, run_number,1])
    ...
    for run_number in range(num_runs):
        for depth in depths:
            args.append([depth, run_number,2])

    unrolled_results_LG_2nd_order = p.map(LG_donut_PSF, args)               # We need to roll up the results into lists later, with each list containing the contrast for all depths for a given instance of tissue.
    ...
    for run_number in range(num_runs):
        ...
        LG_result_2nd_order.append(copy.deepcopy(Results_class(parameters,tmp_field_exports_LG_2nd_order)))
    
    np.save('Results/Contrast_LG', LG_result)
    np.save('Results/Contrast_LG_2nd_order', LG_result_2nd_order)
